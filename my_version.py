...
EMP = {
 "A123456789": {"name":"查理","birthday":"20000101","email":"fantasticharlie@gmail.com","dept":"總經理室","role":"Boss"},
 "B123456789": {"name":"永和","birthday":"20000202","email":"ned5438@gmail.com","dept":"研發部","role":"Employee"},
 "C123456789": {"name":"思妤","birthday":"20000303","email":"jennysha26@gmail.com","dept":"研發部","role":"Employee"},
 "D123456789": {"name":"韋丞","birthday":"20000404","email":"Weicheng0307@gmail.com","dept":"人事部","role":"Employee"},
 "E123456789": {"name":"哲豪","birthday":"20000505","email":"ALIVEcso62@gmail.com","dept":"業務部","role":"Employee"},
}
...
def hit(room, d, s, e, skip=""):
    ...
    for x in db()["bookings"]:
        if x["id"] != skip and x["room"] == room and x["date"] == d and a < dtime(d, x["end"]) + dt.timedelta(minutes=30) and b > dtime(d, x["start"]) - dt.timedelta(minutes=30): return True
    return False
...
def time_grid(room, d):
    ...
    def is_slot_busy(s):
        t_target = dtime(d, s)
        for b in bookings:
            b_start = dtime(d, b["start"]) - dt.timedelta(minutes=30)
            b_end = dtime(d, b["end"]) + dt.timedelta(minutes=30)
...
def mail(to, sub, body):
    ...
    if not user or not pwd:
        st.info(f"[測試模式未寄信] {to}｜{sub}"); return False
...
def mail_booking(b, sub="會議通知"):
    for eid in b["people"]:
        p = EMP[eid]
        mail(p["email"], f"{sub}：{b['title']}", f"{p['name']}，您於 {b['date']} {b['start']} 有一場關於 {b['title']} 的會議，發起人為 {b['org_name']}。")
def report_body(r):
    rr = r["report"]
...
def ai_report(text):
    ...
    try:
        ...
        res = genai.Client(api_key=os.getenv("GEMINI_API_KEY")).models.generate_content(
            model="gemini-2.5-flash", contents=prompt,
            config=types.GenerateContentConfig(response_mime_type="application/json", response_schema=GeminiMeetingReport))
...
with t1:
    ...
    if st.button("確認預約"):
        if not start or not end:
            st.error("預約失敗：當前選擇的時段無法預約，請重新選擇有效的時間。")
        else:
            err = valid(date, start, end) or ("預約失敗：該時段與既有預約或前後 30 分鐘緩衝期衝突。" if hit(room, date, start, end) else "")
            if err:
                st.error(err)
                st.session_state.pop("force_book", None) # 清除二次確認狀態
            elif st.session_state.uid not in people:
    ...
    for b in [x for x in data["bookings"] if x["org"] == st.session_state.uid]:
        with st.expander(f"{b['id']}｜{b['title']}｜{b['date']} {b['start']}-{b['end']}"):
            ...
            c1, c2 = st.columns(2); ns = c1.selectbox("新開始", ts(), ts().index(b["start"]), key="s"+b["id"]); ne = c2.selectbox("新結束", ts(), ts().index(b["end"]), key="e"+b["id"])
...
with t3:
    ...
    for b in visible:
        ...
        for r in [x for x in db()["reports"] if x["booking_id"] == b["id"]]:
            st.caption(f"摘要：{r['report']['meeting_title']}｜is_confirmed={r['is_confirmed']}｜sent={r['sent']}")